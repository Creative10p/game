...
class Cross_sprite(pygame.sprite.Sprite):  # спрайт
    image = load_image(r"images\cross.png")
    ...
    def render(self, screen):  # отрисовка спрайта
        screen.blit(self.image, self.rect)

    def rotate(self):  #  поворот спрайта
        self.rect.topleft = (self.rect.topleft[0]+10, self.rect.topleft[1]+10)
        self.image = load_image(r"images\cross_3.png")

    def update(self):  # перемещение спрайта
        self.rect.y += 15

    def check_place(self):  # проверка на необходимость перемещения спрайта
...
class Zero_sprite(pygame.sprite.Sprite):  # спрайт
    image = load_image(r"images\zero.png")
...
class Scene_cross:  # сцена с отображением победителя(крестик)
    def draw(screen):
...
class Scene_zero:  # сцена с отображением победителя(нолик)
    def draw(screen):
        pygame.display.set_mode((600, 630))
        screen.fill('black')
...
class Scene_draw:  # сцена с отображением исхода матча(ничья)
    def draw(screen):
        pygame.display.set_mode((600, 630))
        screen.fill('black')
        for i in range(256):
...
while running:  # главный игровой цикл
    ...
    if fend:
        ...
        string_rendered = font.render("Нажмите любую кнопку, чтобы вернуться на главный экран", True, "white")
