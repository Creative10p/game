...
class Cross:  # класс для отображения крестиков
...
class Zero:  # класс для отображения ноликов
    def __init__(self, position):
    ...
    def action(self, position=(0, 0)):
        ...
        if self.current_player == 'real player':  # ход человека
            ...
            if self.map[self.render_position[0]][self.render_position[1]] == " " and not game_over:
                ...
                self.action()  # вызываем функцию снова, чтобы сходил ии
        elif not game_over:  # ход ии
            self.x, self.y = -1, -1  # координаты хода ии
            if not first_move_made:  # если первым ходит ии
                self.x, self.y = 1, 1
            # далее ходы по стратегии для игры в крестики нолики
            elif self.moves == 2 and self.map[1][1] == self.ai_sign:
                if self.map[0][1] == self.real_player_sign or self.map[2][1] == self.real_player_sign:
                    self.next_step = (0, 0)
                if self.map[1][0] == self.real_player_sign or self.map[1][2] == self.real_player_sign:
                    self.next_step = (2, 2)
                self.x, self.y = 0, 2
            elif current_player != "AI" and self.map[1][1] == "" and self.moves == 1:
                self.x, self.y = 1, 1
            elif current_player != "AI" and self.moves == 1:
                if self.map[0][0] == " ":
                    self.x, self.y = 0, 0
                elif self.map[2][0] == " ":
                    self.x, self.y = 0, 2
            elif self.next_step is not None and self.map[0][2] == self.real_player_sign:
                self.x, self.y = self.next_step[0], self.next_step[1]
                self.next_step = None
            if self.x == -1:   # если ход не найден
                for i in range(3):  # проверка на возможную победы по горизонталям и вертикалям
                    if self.map[i][0] == self.map[i][1] == self.ai_sign and self.map[i][2] == " ":
                        ...
                        self.x, self.y = i, 0
            if self.x == -1 and self.map[1][1] != " ":  # проверка на возможную победу или поражение по диагоналям
                if self.map[0][0] == self.map[1][1] and self.map[2][2] == " ":
                    ...
                    self.x, self.y = 0, 0
            if self.x == -1:
                for i in range(3):  # провека на возможное поражение по горизонталям и вертикалям
...
while running:  # главный игровой цикл
    ...
    if game_over and not end:
        ...
        with open(r'results_of_game\results.txt', 'w') as f:
            ...
            f.write('\n')
            f.write(winner)
        con = sqlite3.connect(r"results_of_game\progress.db")  # подключаем бд
        cur = con.cursor()
